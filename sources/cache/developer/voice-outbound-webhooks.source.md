...
- **Accepted**: when the network accepts the call offered by <<prodname>> and placed to end-user.
...
- **Trombone Connected**: when the <<prodname>> platform is sending a notification to the user and the user asks to talk to an agent. The Trombone Connect event occurs when both parties A (end-user) and B (agent) are connected.
...
### **Usecases for Outbound Calls from <<prodname>>**

When an outbound call is placed from the <<prodname>> platform, see the progression of events in the following use cases. The call event/delivery status is denoted in square brackets and the description of the event is denoted in parentheses.

[block:parameters]
{
  "data": {
    "h-0": "Scenario",
    "h-1": "Sequence of Events",
    "0-0": "Full message heard by the user and the call ended after running through the full flow.",
    "0-1": "OFFERED - ACCEPTED - ANSWERED - [DROPPED - RELEASED]  \n  \nThe events DROPPED and RELEASED are fired simultaneously with the same timestamp.",
    "1-0": "The user cuts the call while playing.",
    "1-1": "OFFERED - ACCEPTED - ASNWERED - [RELEASED - DISCONNECTED (call disconnected) -DROPPED]  \n  \nThe events RELEASED, DISCONNECTED, and  DROPPED are fired simultaneously with the same timestamp.",
    "2-0": "User is busy on another call.",
    "2-1": "OFFERED - ACCEPTED - [DROPPED - REJECTED (no answer)]  \n  \nThe events DROPPED and REJECTED are fired simultaneously with the same timestamp.",
    "3-0": "Number not answered by the user but rang full-cycle.",
    "3-1": "OFFERED - ACCEPTED - [DROPPED - REJECTED (no answer)]  \n  \nThe events DROPPED and REJECTED are fired simultaneously with the same timestamp.",
    "4-0": "No answer on the number; call disconnected while ringing.",
    "4-1": "OFFERED - ACCEPTED - [DISCONNECTED- DROPPED - REJECTED (customer busy)]  \n  \nThe events DISCONNECTED, DROPPED and REJECTED are fired simultaneously with the same timestamp."
  },
  "cols": 2,
  "rows": 5,
  "align": [
