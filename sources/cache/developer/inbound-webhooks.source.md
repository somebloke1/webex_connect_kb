Inbound Webhooks are unique endpoints that can be embedded into your applications to notify <<prodname>> of events occurring in the your application(s).
...
[block:image]
{
  "images": [
    {
      "image": [
        "https://files.readme.io/8d339bb-9b3e6f9a3d0578b5e3303f6034647375.png",
        "9b3e6f9a3d0578b5e3303f6034647375.png",
        "Screenshot of Creating Inbound Webhook."
      ],
      "align": "center",
      "caption": "Screenshot of Creating Inbound Webhook."
    }
  ]
}
[/block]
...
When authentication is enabled you need to use one of service key or JWT credentials associated with the relevant <<prodname>> service which contains the rule/flow you've associated the inbound webhook with.
...
> 
> Service Key or JWT Tokens enables you to validate that the request is being made by an authenticated entity. Signature Validation enables you to ensure the integrity of data and ensure that it wasn't intercepted during the transition from the source till it's received on <<prodname>>.
> 
...
[block:image]
{
  "images": [
    {
      "image": [
        "https://files.readme.io/8146000-Inbound_Webhooks_Signatures.png",
        "Inbound_Webhooks_Signatures.png",
        "Screenshot of Managing Inbound Webhook Integration Page."
      ],
      "align": "center",
      "caption": "Screenshot of Managing Inbound Webhook Integration Page."
    }
  ]
}
[/block]
...
Please note that the service key / JWT token will need to be passed in the header of the request, with header name as _key_.

```json Sample JSON for Inbound Webhook
{  
   "data":[  
      {  
         "type":"articles",
...
> 
> <<prodname>> uses the standard '.'(Dot) notation to parse parameters from arrays in JSON and XML.
> 
...
[block:textarea]
{
  "text": "Inbound Webhooks are asynchronous. When you send a request to <<prodname>> inbound webhooks, <<prodname>> returns an immediate response code and description to confirm whether your request has been successfully queued or not. Here's the list of various response codes with their description.",
  "sidebar": true
}
[/block]
...
| :------------ | :------------------------------------------------------- | :------------------------------------------------------------------------------ |
| 1002          | Queued                                                   | Returned when the request is accepted by <<prodname>>.                          |
...
> 📘 Know Your Endpoint
> 
> Based on the domain you use to log in to <<prodname>>, the endpoint for your API varies. See the [endpoint](https://developers.webexconnect.io/reference/know-your-api-endpoints) section to understand which endpoint to use for your domain.