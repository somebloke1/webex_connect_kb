...
<<prodname>> allows you to use JWT tokens for API authentication as an alternative in addition to the 'Service Key'-based authentication.
...
**iss ** : a claim that is a string identifying the principal that issued the JWT. This value is always the Service ID (this is available on the API tab within a Service in <<prodname>> when you select JWT Token as the Auth Type) when exchanging messages.
...
Decode the base64 encoded 'SERVICE SECRET' which is accessible within the API tab when you select JWT Auth type and use it to generate the signed bearer token (that is to be passed to <<prodname>> for API authentication as a header parameter) using the HS256 algorithm.
...
1. Select **JWT Bearer** as the **Auth Type** from the dropdown menu.
   ...
   [block:image]{"images":[{"image":["https://files.readme.io/5753634d67292ec9cf12f942ebdd8ca0c4582d4d760a07ae0f17df3017dc19b0-Screenshot_2024-12-15_052647.png","","Screenshot of JWT Bearer"],"align":"center","border":true,"caption":"JWT Bearer"}]}[/block]
2. Select **HS256** as the **Algorithm** from the dropdown menu.
3. Enter the **Service Secret** in the **Secret **. `Service secret is available on the API tab within a service in <<prodname>> when you select JWT Token as the Auth type`.

   [block:image]{"images":[{"image":["https://files.readme.io/4056482eeb183dc24ecb55a7b21f4e4473dc87ce7025974d3712036b3f1d69b1-Screenshot_2024-12-15_051321.png","","Screenshot of ServiceID and Service Secret"],"align":"center","border":true,"caption":"ServiceID and Service Secret"}]}[/block]
