# Data Streams - Live Chat / In-App Messaging

Source: https://developers.webexconnect.io/reference/data-streams-in-app
...
| clientId | Unique identification  number of the client. | POXXXX3801/3XX2/v2_fc50XXXXXXXXXX5a |  |
| transid | Contains the transaction ID. | d94ba95c-xxxx-4c4e-xxxx-7851028415f4 |  |
| channel | This is “rt” in case of Live Chat and In-App Messaging. | RTM | Common parameter for all inbound event types. |
| extras | Contains the additional information like some user-specific property, or any custom parameter that is required to display the message in a certain fashion within the app | {       <br>"key1": "value1",  <br>"key2": "value2",  <br>"key3": "value3"     <br>} |  |
...
| thread_id | The unique identification of the thread in which the message needs to be published. | "fd321782-xxxx-4107-xxxx-ebfc473f9e0a" |  |
| appid | Contains the application ID. | a_63XXXXXXXXXXXXX000 | Common parameter for all inbound event types. |
| reply_To | Contains the details of the user to whom the message is being sent. | User1 | Message, Attachments and Postback |
| event | Contains the incoming event type. | Incoming Message  <br>OnPostback  <br>OnThreadClosed  <br>Custom |  |
| ts | Timestamp when MO received to Webex Connect. | 2022-01-17T10:59:51.945Z | Common parameter for all inbound event types. |
| datetime | Data and Time on which the message is received. | 2020-02-25T12:40:45.051+05:30 | Postback, OnThread Closed, and Custom Event |
| pciInfo | Contains the PCI information of the user. | droppedAttachmentCount |  |
| deviceId | Unique identification  number of the device. | fc5XXXXXXXXX25a |  |
| version | Version of the In-App REST API used for sending messages. | 1 |  |
| Name | Contains the name of the user. | Tom |  |
...
| extras | Contains the additional information like some user-specific property, or any custom parameter that is required to display the message in a certain fashion within the app | {       <br>"key1": "value1",  <br>"key2": "value2",  <br>"key3": "value3"     <br>} | Common parameter for all outbound message types. |
...
| templateType | Contains the information about the template type.  <br>Generic or Form | "templateType": "form",  <br>  <br>"templateType": "generic", |  |
