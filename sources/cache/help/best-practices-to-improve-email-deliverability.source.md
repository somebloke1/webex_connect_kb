...
In general, we recommend all clients who use <<prodname>> in combination with AWS SES for sending outbound emails to follow the email best practices below:

## Follow-Email Authentication best practices

### Finish Domain Verification

Add a TXT record to your Domain Name System (DNS) to verify with Amazon SES that you own the domain that you will be sending from. Details of TXT records become available on the Email app asset page within <<prodname>> after you configure the AWS SES credentials.

### Set-up DKIM to protect your domain against spoofing

DKIM, which stands for DomainKeys Identified Mail, is an email authentication method designed to detect forged header fields and content in emails. DKIM enables the receiver to check if email headers and content have been altered in transit. If you haven’t done setting up DKIM already, please enable the DKIM Settings in your <<prodname>> Email App Asset Configuration Page and update your domain’s DNS records using the information displayed in app asset configuration page.
...
[block:image]
{
  "images": [
    {
      "image": [
        "https://files.readme.io/e8920bbef037fc950691039e6504e0ed69cb60b959f71f7b37e0a9c38133464c-email_domain_1.png",
        "",
        "An email received without a Custom MAIL FROM configured (see \"mailed-by\" field)"
      ],
      "align": "center",
      "border": true,
      "caption": "An email received without a Custom MAIL FROM configured (see \"mailed-by\" field)"
    }
  ]
}
[/block]
...
[block:image]
{
  "images": [
    {
      "image": [
        "https://files.readme.io/dc1a53756a572c0b054e52f5fa7b61630994c6e6dc82b0fc99999280dc9446ef-email_domain_2.png",
        "",
        "An email received with a Custom MAIL FROM configured (see \"mailed-by\" field)"
      ],
      "align": "center",
      "border": true,
      "caption": "An email received with a Custom MAIL FROM configured (see \"mailed-by\" field)\n\n| Name                               | Type | Value                                    |\n| :--------------------------------- | :--- | :--------------------------------------- |\n| customMAILFROMsubdomain.domain.com | MX   | 10 feedback-smtp.eu-west-2.amazonses.com |"
    }
  ]
}
[/block]
...
> 📘 Dedicated IPs
> 
> If you have opted for Email via AWS SES, you can request dedicated IPs through <<prodname>> support team (refer to the details available within Contact Us page in your <<prodname>> account). At least 2 dedicated IPs are recommended to avoid service disruptions. Each dedicated IP has a limit of 40 TPS (Transactions per Second), i.e., if you have 2 dedicated IPs, you can send emails at 80 TPS, provided that the TPS set on <<prodname>> for the asset-tenant combination is at least 80.  
