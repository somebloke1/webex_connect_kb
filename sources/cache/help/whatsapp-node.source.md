...
[block:image]
{
  "images": [
    {
      "image": [
        "https://files.readme.io/397b4bf-Whatsapp.jpg",
        "Whatsapp.jpg",
        "Screenshot of WhatsApp Node"
      ],
      "align": "center",
      "sizing": "smart",
      "caption": "WhatsApp Node"
    }
  ]
}
[/block]
...
[block:image]
{
  "images": [
    {
      "image": [
        "https://files.readme.io/cf3ede1-WhatsApp.jpg",
        "Whatsapp config.jpg",
...
- **Customer ID **: Primary key of customer profile. A customer profile can be created to link one or more channel-specific identifiers. <<prodname>> intelligently looks up for the corresponding channel identifier under a given customer profile based on the channel context. This allows for cross-channel communication.
...
[block:parameters]
{
  "data": {
    ...
    "h-2": "Maximum Size",
    "0-0": "Audio",
    "0-1": "AAC, AMR, MPEG, OGG  \nNote: For OGG MIME Type, only media codec supported by WhatsApp. Also, OPUS MIME Type is no longer supported by WhatsApp.",
    "0-2": "16MB",
    "1-0": "Document",
    "1-1": "PDF, DOC, DOC(X), PPT, PPT(X), XLS, XLS(X), .TXT",
    "1-2": "100MB",
    "2-0": "Image",
    "2-1": "JPG, JPEG, PNG",
    "2-2": "5MB",
    "3-0": "Video",
    "3-1": "MP4, 3GPP  \nNote: Only H.264 video codec and AAC audio codec is supported for MP4 Mime Type.",
    "3-2": "16MB",
    "4-0": "Sticker",
    "4-1": "WEBP",
    "4-2": "Static stickers: 100KB  \nAnimated stickers: 500KB"
  },
  "cols": 3,
  "rows": 5,
  "align": [
...
[block:image]
{
  "images": [
    {
      "image": [
        "https://files.readme.io/d654221-WhatsApp1.jpg",
        "Whatsapp config1.jpg",
        1192
      ],
      "align": "center",
      "border": true,
...
[block:image]
{
  "images": [
    {
      "image": [
        "https://files.readme.io/e485177-3d1bcfc-sticker_outline.png",
        "3d1bcfc-sticker_outline.png",
        "Screenshot of Sticker Outline"
      ],
      "align": "center",
      "border": true,
      "caption": "Sticker Outline"
    }
  ]
}
[/block]
...
This message type allows you to send proactive messages using a pre-defined template to your users. A template should be registered from Tools > Templates section within <<prodname>> platform. Include the following as part of a notification message using the template message type:
...
[block:parameters]
{
  "data": {
    ...
    "h-1": "Description",
    "0-0": "Template Name",
    "0-1": "Select the required template from the drop-down list. The drop-down list displays all the pre-defined templates for WhatsApp that are already approved.",
    "1-0": "Header Parameters",
    "1-1": "Under **Header Parameters**, enter/select the following:  \n  \n> _ **Parameter Name** - the name of the parameter. This field is auto-populated based on the template you have selected.  \n> _ **Parameter Type** - the type of the parameter. Supported types are _Text_, _Currency_, _Date-time_. Currency and Date time should be used to localize the content based on user's locale.  \n> _ **Media URL** - a URL pointing to the document, video or image attachment.  \n> _ **File Name** -  filename for document attachment.  \n> \\* **Value** - the value of the parameter.",
    "2-0": "Body Parameters",
    "2-1": "Under **Body Parameters**, enter/select the following:  \n  \n> _ **Parameter Name** - the name of the parameter. This field is auto-populated based on the template you have selected. The parameter name is \"OTP\" for Authentication templates.  \n> _ **Parameter Type** - the type of the parameter. Supported types are _Text_, _Currency_, _Date-time_. Currency and Date time should be used to localize the content based on user's locale.\n>\n> > _ **Currency** params- **Code** indicates the code of the language or locale to use; **Amount** is the currency amount multiplied by 1000. E.g, for USD 9.99, specify the amount as 9990; the **Default value** is used when the localization fails.  \n> > _ **Date and time** - Epoch timestamp is the Unix timestamp showing the number of seconds that have elapsed since 00:00:00, 1 January 1970 (UTC), minus leap seconds. [JavaScript](https://jsfiddle.net/tbxac0de/) for converting local time to Unix epoch; the **Default value** is used when the localization fails.  \n> > \\* **Value** - the value of the parameter.The following buttons can only be configured with Authentication category Templates\\> - OTP - the value of the One-time-Password",
    "3-0": "Identity Key Hash",
    "3-1": "Enter the Identity key hash of the customer. Use this to authenticate the identity of the customer. If there is a mismatch of the hash, the message will not be delivered. This is an optional field.",
    "4-0": "Footer (Optional)",
    "4-1": "Optional footer text. This needs to be specified while creating a template.",
    "5-0": "Buttons (Optional)",
    "5-1": "You can see the configured interactive buttons in this section. The interactive buttons can either be a Call-To-Action (CTA) or a Quick Reply (QR) button.  \n**Call-To-Action** (CTA) - Supported types: Call phone and Visit website. In case of Visit website, you can define a dynamic URL path to append with URL domain (domain should be pre-defined while registering a template).  \nYou can have a maximum of one button type of each type per message, i.e, one Call Phone Number button and one Visit Website button. WhatsApp allows a maximum of two CTA buttons.  \n**Quick Replies** (QR) - use this button to configure quick replies like Yes, No, Maybe. You can use a maximum of three QR buttons.  \nDefine a payload to each QR button, this will be received back as part of the Postback event in receive node.  \nThe following buttons can only be configured with Authentication category Templates  \n\\_ **OTP transfer buttons **  \n**Auto-fill**- This button type will include a button for sharing the code with your app when the customer taps the button on their device.  \n**Copy-Code** -This button type will include a button for customers to copy and paste the code into their app."
  },
  "cols": 2,
  "rows": 6,
  "align": [
...
[block:image]
{
  "images": [
    {
      "image": [
        "https://files.readme.io/9da9053-Hash.png",
        "d7b2820-whatsappnotification_image.png",
        "Screenshot of WhatsApp Message Previews for Template Messages"
      ],
      "align": "center",
      "border": true,
      "caption": "WhatsApp Message Previews for Template Messages"
    }
  ]
}
[/block]
...
You can create your own templates, submit them for approval to WhatsApp, and track approval status using <<prodname>>'s Template Management section accessible under 'Tools > Templates'.
...
[block:image]
{
  "images": [
    {
      "image": [
        "https://files.readme.io/332e6aa-3.png",
        "cta_buttons_preview_send_node.png",
        "Screenshot of Interactive Template with CTA Buttons"
      ],
      "align": "center",
      "border": true,
      "caption": "Interactive Template with CTA Buttons"
    }
  ]
}
[/block]
...
[block:image]
{
  "images": [
    {
      "image": [
        "https://files.readme.io/56b11f0-4.png",
        "quick_replies_buttons_preview.png",
        "Screenshot of Interactive Template with QR Buttons."
      ],
      "align": "center",
      "border": true,
      "caption": "Interactive Template with QR Buttons"
    }
  ]
}
[/block]
...
[block:image]
{
  "images": [
    {
      "image": [
        "https://files.readme.io/6f64f7f-WhatsApp25.png",
        "",
        "Screenshot of Authentication Template with Auto-fill button."
      ],
      "align": "center",
      "border": true,
      "caption": "Authentication Template with Auto-fill button"
    }
  ]
}
[/block]
...
[block:parameters]
{
  "data": {
    ...
    "h-1": "Description",
    "0-0": "Formatted Name",
    "0-1": "The full name of the contact as it normally appears",
    "1-0": "Prefix",
    "1-1": "The prefix of the contact like Mr, Ms, Mrs, Dr, Col etc.  \n  \nDo not use punctuation in this field.",
    "2-0": "First Name",
    "2-1": "The first name of the contact",
    "3-0": "Last Name",
    "3-1": "The last name of the contact",
    "4-0": "Middle Name",
    "4-1": "The middle name of the contact",
    "5-0": "Suffix",
    "5-1": "The prefix of the contact like Jr, Sr, PhD, MD, Ret etc.  \n  \nDo not use punctuation in this field.",
    "6-0": "Address(es) (optional)",
    "6-1": "The address or addresses of the contact, if any",
    "7-0": "Email(s) (optional)",
    "7-1": "The email or email addresses of the contact, if any",
    "8-0": "Phone Number(s) (optional)",
    "8-1": "The phone number(s) of the contact, if any",
    "9-0": "Birthday (optional)",
    "9-1": "The birthday fo the contact, if you wish to provide",
    "10-0": "Website(s) (optional)",
    "10-1": "The website of the contact, if any",
    "11-0": "Organization (optional)",
    "11-1": "The organization of the contact, if you wish to provide"
  },
  "cols": 2,
  "rows": 12,
  "align": [
...
[block:image]
{
  "images": [
    {
      "image": [
        "https://files.readme.io/0428ca0-WhatsApp_List.png",
        "WhatsApp List.png",
        "Screenshot of Interactive Messages."
      ],
      "align": "center",
      "border": true,
      "caption": "Screenshot of Interactive Messages."
    }
  ]
}
[/block]
...
You can see the data that this node generates as output [variables](doc:variable-management). These variables are available for use in subsequent nodes. 
...
[block:parameters]
{
  "data": {
    ...
    "h-1": "Outcome name",
    "0-0": "Green",
    "0-1": "_ **onSuccess** - When waiting for the condition is set to \\_none_ and upon successfully executing the send request  \n_ **onSubmit** - When waiting for the condition is set to wait for \\_Gateway Submit_ and upon successfully submitting the message within the set timeout value  \n\\_ **onDeliveryReportSuccess** - When wait for the condition is set to \\_receive delivery report\\* and upon successfully receiving the DR within the set timeout value",
    "1-0": "Yellow / Amber",
    "1-1": "\\* **onTimeout** - When one of the waits for conditions is set and no acknowledgment is received within the timeout window set",
    "2-0": "Red",
    "2-1": "_ **onError** - When node execution failed. Any errors arising from passing invalid field values  \n[List of response codes](https://developers.imiconnect.io/reference#section-twitter) corresponding to onError outcome  \n_ **onPolicyFail** - When one of the policy checks failed.  \nE.g. Send was attempted after the expiry time  \n\\_ **onDeliveryReportFail** - When wait for the condition is set to receive  \\_delivery report\\* or and when no corresponding DR is received within the timeout window set."
  },
  "cols": 2,
  "rows": 3,
  "align": [
