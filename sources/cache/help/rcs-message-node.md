...
Source: https://help.webexconnect.io/docs/rcs-message-node
...
Use the [RCS Capability Node](https://help.webexconnect.io/docs/rcs-capability-node) as a predecessor to this node to know if the user's mobile device has RCS capability.



![RCS Message Node](https://files.readme.io/66bb756-rcs_message.png)
...
![RCS Message Configuration Window](https://files.readme.io/6642d1c-RCS2.jpg)
...
| Thumbnail | • Max file size: 100 kB (Recommended ≤50 kB)  <br>• Dimensions: 800 × 800 px, square  <br>• JPG/PNG |
| Media (Images, Video, PDF Preview) | • Heights: 112 DP (Short), 168 DP (Medium), 264 DP (Tall)  <br>• Total card payload limit: 250 kB |
| Suggested Replies | • Up to 4 per card  <br>• Max 25 characters each |
| Suggested Actions | • Up to 4 per card  <br>• Max 25 characters each |
| Chip List Suggestions | Up to 11 suggestions below carousel |
| Carousel | • Up to 10 cards  <br>• Small card: 542 DP height, 180 DP width  <br>• Medium card: 592 DP height, 296 DP width |
...
You can see the data that this node generates as output [variables](https://help.webexconnect.io/docs/variable-management-in-flows). These variables are available for use in subsequent nodes. The standard output variables for this node are:
...
- **deliveryStatusCode** - contains the delivery status code. See the [API Statue Code section](https://help.webexconnect.io/docs/references-api-codes) for the status codes
...
| --- | --- |
| Success (green)  <br>  <br>**Note**: You can see this node edge only when you complete the node configuration. | > _ **onSuccess** - the flow exits through this node when it is a success  <br>> _ **onSubmit** - the flow exits through this node when a wait for gateway submit condition is configured and is a success.  <br>> \* **onDeliveryReportSuccess** - the flow exits through this node when a wait for delivery report condition is configured and is a success. |
| Timeout (yellow/amber) | > \* **onTimeout** - the flow exits through this node when a timeout occurs |
| Error (red) | > _ **onError** - the flow exits through this node outcome when there is an error  <br>> _ **onPolicyFail** - the flow exits through this node outcome when the expiry condition cannot be met  <br>> \* **onDeliveryReportFail** - the flow exits through this node when a wait for delivery report condition is configured and is a failure. |
...
| --- | --- | --- | --- |
| Image | _ BMP  <br>_ GIF  <br>_ JPEG (highly recommended)  <br>_ PNG | 500 kb or less | **Stand-alone Rich Card**  <br>  <br>> _ Short: 1085x310 pixels (3.5:1 ratio)  <br>> _ Medium: 1080x720 pixels (1.5:1 ratio)**Rich Card in Carousel**  <br>> 1080x787 pixels (1.37:1) is the recommended resolution |
| Video | MPEG-4 |  |  |
| Audio | _ MP4  <br>_ M4A  <br>_ MP3  <br>_ WAV |  |  |
...
For example, you start node trigger [Inbound Webhooks](https://help.webexconnect.io/docs/inbound-webhooks) or [HTTP Request Node](https://help.webexconnect.io/docs/http-request-node) may return a dynamic array response as a part of customer journey.



![Dynamic Carousel in RCS Send Node](https://files.readme.io/404a096-RCS3.jpg)
