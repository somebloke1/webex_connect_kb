...
Source: https://help.webexconnect.io/docs/navigate-flow-builder-canvas
...
![Flow Builder Interface](https://files.readme.io/7e3dff1-flow_designer.png)
...
This is the region where you can build and configure a flow using [Nodes](https://help.webexconnect.io/docs/nodes#section-list-of-nodes). The flow canvas allows you to drag-and-drop nodes from the _Node Palette_ and connect the nodes using connectors to build a flow.

### 2. Node Palette

The node palette lists all the available [Nodes](https://help.webexconnect.io/docs/nodes#section-list-of-nodes) including pre-built nodes and custom nodes. These nodes are categorized into logical groups based on their functions. 

- _Utilities_ - Standard functions for orchestrating your business process 
- _Channels_ - Nodes for embedding Send/Receive message events through any of the Webex Connect channels
...
Access Documentation, [Flow Settings](https://help.webexconnect.io/docs/flow-settings), and tools for managing edits made to the flow. 
...
This area displays the current [version](https://help.webexconnect.io/docs/flow-version-history) of the flow. Click the drop-down list box to view the _Version History_ and to load any of the available versions in the flow canvas.
...
- _[Share](https://help.webexconnect.io/docs/flows-introduction#section-sharing-a-flow)_: Generates a read-only link to share the flow with other users including/excluding the **Analyze Mode**. The Analyze Mode option is visible only if the administrator has configured it. You must protect the shareable link with a password. 
- _[Flow Debug](https://help.webexconnect.io/docs/transaction-logs-and-debugging)_: Launches the debug pane from the bottom of the flow and allows you to access detailed debug logs for each flow run.