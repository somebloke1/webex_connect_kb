The <<prodname>> platform supports web-based Single Sign-On (SSO) using the Security Assertion Markup Language (SAML). This feature allows <<prodname>> enterprise clients to enable their employees to log in seamlessly using their existing organizational credentials.

Single Sign-On (SSO) is an integrated feature of <<prodname>> and must be set up to allow users to authenticate via their organizational credentials. Please contact your account manager to assist with enabling the SSO feature for your organization.
...
Obtain the following assertion data from the <<prodname>> service provider:
...
[block:image]
{
  "images": [
    {
      "image": [
        "https://files.readme.io/b52161102574f92501f454a59cf6f6fc257c67e74b82f6e43d14f4e91f4da3d3-Single-Sign-On_1.png",
        "Img1.png",
        "SSO Settings - Part 1"
      ],
      "align": "center",
      "border": true,
      "caption": "SSO Settings - Part 1"
    }
  ]
}
[/block]
...
[block:image]
{
  "images": [
    {
      "image": [
        "https://files.readme.io/1a6b52fae0711a76a22ada45bff9dabf5b7df4e48bb796dddae6ace28cf26bca-Single-Sign-On_2.png",
        "",
        "SSO Settings - Part 2"
      ],
      "align": "center",
      "border": true,
      "caption": "SSO Settings - Part 2"
    }
  ]
}
[/block]
...
5. Provide the **Identity Provider Login URL**. This is the URL to which <<prodname>> sends a SAML request to start the login sequence.
...
10. Authentication Context Requirements:  Provide the list of Allowed Authentication Context values (`SAML AuthnContextClassRef`) that <<prodname>> will include in the authentication request to the Identity Provider (IdP). This setting is mandatory and is pre-populated by default for both existing and new tenants with:  
...
11. Provide **<<prodname>>Service Provider Details**. These details are required to configure the federated authentication, a single sign-on method that uses SAML assertions sent to an <<prodname>> endpoint.

- **Assertion Customer Service URL** - the URL that the identity provider uses to verify SAML messages from <<prodname>>.
...
[block:image]
{
  "images": [
    {
      "image": [
        "https://files.readme.io/f0c64f6-sso-table.png",
        "sso-table.png",
        "Image illustrating login processes for existing and new users with password and SSO."
      ],
      "align": "center",
      "border": true,
      "caption": "Image illustrating login processes for existing and new users with password and SSO"
    }
  ]
}
[/block]
...
<<prodname>> allows you to test your SSO configuration by simply clicking a Test button after you have configured the SSO settings. Enter the Login Email ID of the user for whom you have enabled and want to test the SSO option for. 
...
[block:image]
{
  "images": [
    {
      "image": [
        "https://files.readme.io/49dcc8d6c964a568412f3e5b38b46fe2f1a03787308fc9b999b98e59d1062a6d-Single-Sign-On_2.png",
        "SSO.jpg",
        "Screenshot of Test Single Sign-On page"
      ],
      "align": "center",
      "border": true,
      "caption": "Test SSO"
...
[block:image]
{
  "images": [
    {
      "image": [
        "https://files.readme.io/c5c12743ce07bd704b8638ed93dafbeed244b58103a8bd344648de7399f7eb2e-2025-03-28_11-27-20.jpg",
        "SSO-testpage.png",
        "Screenshot of Test Single Sign-On page"
      ],
      "align": "center",
      "border": true,
      "caption": "Test Single Sign-On"
    }
  ]
}
[/block]
...
The user will be directed to the login page of the entered ‘Identity Provider’s Login URL’. In case of a successful login, the user will be provided a ‘test session’ with <<prodname>>.  
...
[block:image]
{
  "images": [
    {
      "image": [
        "https://files.readme.io/c48d17c-errormsg.png",
        "errormsg.png",
        "Image showing an error message indicating that entered data does not match expected values due to an incorrect ID, with the message provided by the Identity Provider (IDP)."
      ],
      "align": "center",
      "border": true,
      "caption": "Screenshot displaying error message for mismatched data due to wrong ID, as received from IDP."
    }
  ]
}
[/block]
...
<<prodname>> has introduced Remote Logout functionality which allows a user that is active on multiple platforms/products to logout at once from everywhere simply from a single platform.
...
Remote logout URL is used to terminate session at IDP's end when the user logs out of <<prodname>>(SP).”

On successful logout from <<prodname>> for SSO enabled users, a session termination request is sent on remote logout URL.

Remote logout URL is used to terminate session at IDP end when user logs out of <<prodname>>(SP).

<<prodname>> (SP) will not delete any other active SP's session (other than <<prodname>> when SSO user logouts of <<prodname>>) and IDP should take action to delete other SP's sessions to achieve Single Logout.