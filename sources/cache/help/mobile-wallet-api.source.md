...
[block:image]
{
  "images": [
    {
      "image": [
        "https://files.readme.io/7c7da52-15598ce0-8448-4ddb-8d66-9bc77f070077.png",
        "15598ce0-8448-4ddb-8d66-9bc77f070077.png",
        "Screenshot displaying the interface for designing a mobile wallet pass for Google Android Pay"
      ],
      "align": "center",
      "caption": "Screenshot displaying the interface for designing a mobile wallet pass for Google Android Pay"
    }
  ]
}
[/block]
...
[block:parameters]
{
  "data": {
    ...
    "h-1": "Description",
    "h-2": "Required",
    "0-0": "`data.customHeaderFields[N].key` `data.customHeaderFields[N].label`  `data.customHeaderFields[N].value`  `data.customHeaderFields[N].align`",
    "0-1": "Header fields are displayed at the top of the pass.",
    "0-2": "no",
    "1-0": "`data.customPrimaryFields[N].key`  \n`data.customPrimaryFields[N].label`  `data.customPrimaryFields[N].value`  `data.customPrimaryFields[N].align`",
    "1-1": "Primary fields are displayed on the strip image of the pass",
    "1-2": "no",
    "2-0": "`data.customSecondaryFields[N].key`  `data.customSecondaryFields[N].label`  \n`data.customSecondaryFields[N].value`  \n`data.customSecondaryFields[N].align`",
    "2-1": "Secondary fields are located below the strip image.",
    "2-2": "no",
    "3-0": "`data.customAuxiliaryFields[N].key` `data.customAuxiliaryFields[N].label`  \n`data.customAuxiliaryFields[N].value` `data.customAuxiliaryFields[N].align`",
    "3-1": "Auxiliary fields are located below the secondary fields.",
    "3-2": "no",
    "4-0": "`data.customBackFields[N].key`  `data.customBackFields[N].label`  `data.customBackFields[N].value`  `data.customBackFields[N].align`",
    "4-1": "Back fields are located on the back of the pass.",
    "4-2": "no"
  },
  "cols": 3,
  "rows": 5,
...
[block:parameters]
{
  "data": {
    ...
    "h-2": "Required",
    "0-0": "passId",
    "0-1": "The ID of the wallet pass/object to update.",
    "0-2": "yes",
    "1-0": "customLayout",
    "1-1": "Must be included but the value portion can be left blank",
    "1-2": "Yes",
    "2-0": "data",
    "2-1": "A nested object that contains the data elements and values of the wallet object/pass. All data fields (except for type) must be specified including values that are not changing. Additional data fields may be included.",
    "2-2": "yes",
    "3-0": "data.trackProgress",
    "3-1": "Pass this as \"true\" in order to track the progress of a pass upade on the Status report page in wallet builder ui",
    "3-2": "no",
    "4-0": "`data.<field>.changeMessage` (Apple only)",
    "4-1": "A lock screen notification may be triggered on Apple devices to notify the consumer about the update by including a `changeMessage` value. The lock screen notification only displays when a field value is modified.  \nNotes about using `changeMessage`:  \n  \n1. Use %@ in the changeMessage to display the new value of the field within the lock screen notification message.\n2. If `%@` is omitted from the `changeMessage`, the notification message will say \"Store card changed\" or \"Coupon changed\" (depending on the pass type) regardless of any other text in the `changeMessage`.\n3. `%@` only applies to the field value (for example, an updated label will not trigger a lock screen notification).\n4. Multiple fields may be updated, but the lock screen notification is only capable of displaying information about one of the fields.",
    "4-2": "no"
  },
  "cols": 3,
  "rows": 5,
