...
<<prodname>> chat engine offers a pre-built connector to integrate with Cisco ECE and enable customer support over digital messaging channels such as SMS, <<AMB>>, and WhatsApp.

## Channel Support

<<prodname>>'s Cisco ECE Integration supports the following channels:
...
The key capabilities of the Cisco ECE Integration within <<prodname>> are as follows:
...
- Trigger pre-chat and post-chat flows on <<prodname>> from Cisco ECE
...
- Chatbot integration for customer self-service  
  Enable customer self-service over supported channels by integrating <<prodname>> with Bot or any other third-party chatbot with a seamless handover from chatbots to live agents on Cisco ECE when human intervention is needed.
- Automated Language detection  
  <<prodname>> supports automatic language detection and routing of messages to relevant teams on the Cisco ECE.
- System messages  
...
To set up a Cisco ECE integration in <<prodname>>:

1. Sign in to <<prodname>> with your credentials.
...
[block:image]
{
  "images": [
    {
      "image": [
        "https://files.readme.io/5e17adc-CiscoECE.jpg",
        "CiscoECE.jpg",
        "Screenshot of List of Integrations."
      ],
      "align": "center",
      "sizing": "auto",
      "caption": "List of Integrations"
    }
  ]
}
[/block]
...
[block:image]
{
  "images": [
    {
      "image": [
        "https://files.readme.io/cd70545-ece_authentication_details.png",
        "ece_authentication_details.png",
...
[block:image]
{
  "images": [
    {
      "image": [
        "https://files.readme.io/7a75b37-entry_point_configs.png",
        "entry_point_configs.png",
        "Screenshot of Entry Point Configurations"
      ],
      "align": "center",
      "sizing": "80",
      "caption": "Entry Point Configurations"
    }
  ]
}
[/block]
...
[block:image]
{
  "images": [
    {
      "image": [
        "https://files.readme.io/9e49169-messages_for_events.png",
        "messages_for_events.png",
        "Screenshot of Configure Messages Section."
      ],
      "align": "center",
      "sizing": "smart",
      "caption": "Configure Messages"
    }
  ]
}
[/block]
...
9. Click **Add Entry Point** and repeat the above steps to add other entry points to the integration. This is an optional step.  
   You can add entry points in scenarios where multiple entry points are associated with a single Cisco ECE integration in <<prodname>>.
...
[block:image]
{
  "images": [
    {
      "image": [
        "https://files.readme.io/2f4c506-CustomAttributes.jpg",
...
11. Click **Save** to test and save the integration.  
    This step verifies if <<prodname>> can connect to Cisco Finesse chat console and retrieve an access token successfully. An error message appears for any connection issues.

## Use the Cisco ECE Integration

Once you’ve integrated Cisco ECE with <<prodname>>, you can test the end-to-end experience from receiving a customer message through any of the supported channels to facilitating a two-way chat communication between an agent and the customer by following the below steps:

1. Integrate Cisco ECE with <<prodname>>.
2. Configure the channels that you want to use with Cisco ECE.
3. Create a new service in <<prodname>> to use this integration.
4. Map the Cisco ECE Integration as a CCSP endpoint within the service. Define logic to route the customer messages to CCSP in one of the following ways:
   - [Create a rule](doc:rules) to forward the customer messages to Cisco ECE.
   - [Create a flow](doc:flows#section-creating-a-flow) that uses the CCSP (Create Chat and Validate Chat) nodes. Publish the flow and then launch it.

When a customer sends a message through the configured communication channel, the message reaches the agents on Cisco ECE. When an agent responds, Cisco ECE notifies <<prodname>> of any outbound messages that need to be sent to the customers. Based on the conversation object, <<prodname>> automatically detects the original channel and pushes messages to the customer on that channel.

## Use Cisco ECE Integration for Customer Support over Messenger

In this sample tutorial, you can see how to integrate Messenger to Cisco ECE through <<prodname>> chat engine to allow direct communication between an agent and a customer.

1. Create a Cisco ECE integration with the name _MyCiscoECEIntegration_. For more information, see the [Integrate Cisco ECE with <<prodname>>](#section-integrate-cisco-ece-with-imiconnect) section.
...
[block:image]
{
  "images": [
    {
      "image": [
        "https://files.readme.io/398dc90-create_messenger_app.png",
        "create_messenger_app.png",
        "Screenshot of Add Messenger Page."
      ],
      "align": "center",
      "caption": "Add Messenger Page"
    }
  ]
}
[/block]
...
For more information, see [Creating a Messenger App](doc:facebook-messenger#section-creating-a-facebook-messenger-messenger-app).
...
[block:image]
{
  "images": [
    {
      "image": [
        "https://files.readme.io/097fc52-rule_select_event.png",
        "rule_select_event.png",
        "Screenshot of Selecting the Incoming Event."
      ],
      "align": "center",
      "sizing": "80",
...
[block:image]
{
  "images": [
    {
      "image": [
        "https://files.readme.io/2a7cd7f-rule_select_action.png",
        "rule_select_action.png",
        "Screenshot of Selecting Action Forward to CCSP"
      ],
      "align": "center",
      "sizing": "80",
      "caption": "Select Action - Forward to CCSP"
    }
  ]
}
[/block]
...
[block:image]
{
  "images": [
    {
      "image": [
        "https://files.readme.io/e75f90a-rule_details.png",
        "rule_details.png",
        "Screenshot of Rule Details Section."
      ],
      "align": "center",
      "sizing": "80",
...
In this sample tutorial, you can see how to send and receive messages to Cisco ECE chat console on the SMS channel through the <<prodname>> chat engine.
...
This rule sends all the mobile originating message through the SMS channel (to the configured number with the defined keyword) to the Cisco ECE chat console. <<prodname>> sends all the messages from Cisco ECE to the customer in the form of SMSes.
...
2. The message reaches the Cisco ECE chat console through <<prodname>> and a pre-configured welcome message appears in the Messenger window. For example, <code>Thank you for contacting us. We are here to help you resolve your issue. Please be patient until an agent attends you</code>.
...
1. A customer sends a message on the Cisco ECE Integration Facebook page through Messenger.
2. The message reaches the Cisco ECE chat console through <<prodname>> and a pre-configured welcome message appears in the Messenger window.
