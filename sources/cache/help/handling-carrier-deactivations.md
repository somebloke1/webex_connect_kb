...
Source: https://help.webexconnect.io/docs/handling-carrier-deactivations
...
| State    | Consent for Messages |
| :------- | :------------------- |
| DEACT    | Changes to opt out   |
| TRANSFER | Changes to opt out   |
| SUSPEND  | No change            |
| RESUME   | No change            |

## DEACT State

When a user’s phone number is deactivated, the number is put in the DEACT state, and the consent immediately changes to opt-out. That means the user will be removed from all the groups.
...
If the Carrier Lookup service is unavailable, Contact Policy retries the lookup for the number up to three times within 24 hours.

If the service does not return a response after 24 hours, or returns `BadData`, `Error`, `null`, or `Unknown`, Contact Policy marks the number as **Deact Failed** and logs the record in Contact Policy Export Logs.

Because the lookup process does not complete successfully, these records do not include a mapped Contact Policy Consent Group ID or Contact Policy Consent Group Name. You must opt out the number manually.

> 📘 Note
> 
> Carrier Deactivation failure details are now included in Contact Policy Export Logs. This feature is currently available exclusively for the Oregon region and requires the Contact Policy feature to be enabled. Please contact support to enable this logging functionality.