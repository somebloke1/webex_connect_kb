...
Source: https://help.webexconnect.io/docs/flows-introduction
Documentation version: 6.20.0
Retrieved: 2026-09-08T23:28:41+00:00

Webex Connect Flow Builder allows you to configure multi-step customer interaction flows with minimal programming or scripting efforts. It has an easy-to-use drag and drop interface called _Flow Canvas_, which allows you to build communication flows using pre-built building blocks referred to as [Nodes](https://help.webexconnect.io/docs/nodes). The communication logic is built on the flow canvas by connecting and configuring nodes in a logical order. 
...
![Flow Creation Wizard](https://files.readme.io/76d8dca-9.jpg)




Depending on the Webex Connect version you're on, you may see one of both of the following options:
...
- The flow builder screen appears with a [Start](https://help.webexconnect.io/docs/start-node) node. Setup the Start node following the on-screen instructions.
- Configure business logic by [adding nodes to flow canvas](https://help.webexconnect.io/docs/nodes#section-adding-a-node-to-flow-canvas) by dragging them from the [Node Palette](https://help.webexconnect.io/docs/navigate-flow-builder-canvas#section-2-node-palette) onto the [Flow Canvas](https://help.webexconnect.io/docs/navigate-flow-builder-canvas#section-1-canvas).
- Setup each Node following the [Node Configuration](https://help.webexconnect.io/docs/nodes#section-node-configuration)
- Set Flow [Settings ](https://help.webexconnect.io/docs/flow-settings)appropriately, map [logbook(s)](https://help.webexconnect.io/docs/logbook) as needed to capture [custom logs](https://help.webexconnect.io/docs/flow-settings#section-custom-logs) in the flow, and define the [Flow Outcomes](https://help.webexconnect.io/docs/flow-settings#section-flow-outcomes).
...
- Stitch the Nodes together using [Connectors](https://help.webexconnect.io/docs/nodes#section-drawing-a-connector), building the flow business logic. 
...
- You can SAVE your flow at any stage during the configuration. Saving a flow also lists any configuration errors or warnings - which are displayed in the [Errors & Warnings](https://help.webexconnect.io/docs/flows-introduction#section-errors-and-warnings) pane at the right side of the flow canvas.
- Click on 'Make Live' to launch the make live wizard if the flow doesn't have any errors.  From this wizard you can now manage: a. Apps / Numbers for the channels used in the flow. Apps will only need to be selected if you have used a send or receive node and selected one of the app-based channels such as Mobile/Web apps, Messenger, Email, etc., and b. Define values for [Custom Variables](https://help.webexconnect.io/docs/variable-management-in-flows) you have created in the flow. Additionally, you can add a comment against the flow version to track what changes were deployed with each increment.

When you 'Save' a flow, it is saved as 'Working Draft' and when you make the flow live, the flow status is changed to 'Live'. For more information, see [Flow Versioning](https://help.webexconnect.io/docs/flow-version-history).
...
5. Now make the necessary modifications to the flow and click **Save** or **Make Live** accordingly. For more information, see [Flow Versioning](https://help.webexconnect.io/docs/flow-version-history).
...
![Share Flow](https://files.readme.io/df48633-create_flow1.jpg)
...
> 
> A media file folder containing the audio prompts must be created using [Tools - Voice media ](https://help.webexconnect.io/docs/voice-media) prior to creating a voice flow. However, the audio prompts contained within the designated media folder can be updated later and the same would be reflected within the voice flow.

To configure a voice flow: 

- You will need to set the 'Media Folder' containing the Voice media to be used in the flow. You can manage your voice media using [Tools - Voice media ](https://help.webexconnect.io/docs/voice-media).
...
> 
> - Every flow must begin with a [Start node](https://help.webexconnect.io/docs/start-node).
