...
| :--------------------------------------------------------------------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------- | :------------------- | :------------------------- |
| [Create Task](doc:create-task)                                               | Calls Webex CC Task API to Create a new Task at WxCC using a unique identifier.                                                                                        | When a new conversation is initiated by an end customer, or an existing conversation is reopened.                                                   | Create Task          | v1.2 and v1.0              |
...
| [Queue Task](doc:queue-task)                                                 | Calls Webex CC Task API to Queue a Task.                                                                                                                               | When you want to escalate a conversation to a customer service agent in <<WebexCC>>                                                                 | Queue Task           | v1.0, v1.1, v1.2, and v1.3 |
| [PIQ and EWT](doc:piq-and-ewt)                                               | Provides the caller's current Position in Queue (PIQ) and the Estimated Wait Time (EWT).                                                                               | When you want to inform a customer who is waiting to chat with an agent about the estimated wait time and there position in queue.                  | PIQ and EWT          | v1.1                       |
| [Routed Notification](doc:routed-notification)                               | Calls Webex CC Task API to notify that that the task routed is accepted when the participant is added successfully.                                                    | After receiving a confirmation from <<WebexCC>> on whether a routing request (i.e., addition of participant to a conversation) was accepted or not. | Routed Accepted      | v1.1                       |
|                                                                              | Calls Webex CC Task API to notify that that the task routed is rejected when adding the participant failed.                                                            | After receiving a confirmation from <<WebexCC>> on whether a routing request (i.e., addition of participant to a conversation) was rejected or not. | Routed Rejected      | v1.1                       |
| [Modify Notification](doc:modify-notification)                               | Calls Webex CC Task API to notify that that the task modified accepted when the participant is added/removed successfully based on the context.                        | After receiving a confirmation from <<WebexCC>> on whether a task modification request (such as conversation transfer) was accepted or not.         | Modified Accepted    | v1.1                       |
|                                                                              | Calls Webex CC Task API to notify that that the task modified rejected when adding/removing the participant failed based on the context.                               | After receiving a confirmation from <<WebexCC>> on whether a task modification request (such as conversation transfer) was rejected or not.         | Modified Rejected    | v1.1                       |
| [Screen Pop](doc:screen-pop)                                                 | Calls Webex CC Screen pop API. While configuring flows in <<prodname>>, you’d need to use this node for sending a screen pop message to the agent contact on Webex CC. | When sending a pop-up message to the agent contact on <<WebexCC>>.                                                                                  | Screen Pop           | v1.1                       |
| [Close Task](doc:close-task)                                                 | Calls Webex CC Task API to close task directly on Webex CC.                                                                                                            | For closing a Task when an agent closes a conversation, or a conversation gets closed due to some unexpected failure.                               | Closed Task on WxCC  | v1.0                       |
...
> 📘 Note
> 
> For all the Authorization that is being accessed by <<prodname>>, if the authorizing credentials are updated in the integrating system, the <<prodname>> will loose access to the integration and the users will have to re-authorize the node.
> 
> Example of Authorization Credentials are: Username, Password, Client ID, Client Secret Key, Access Key etc.

## Node Authorization for <<WebexCC>> Task nodes

For <<WebexCC>> Task nodes  (Task nodes are Create Task, Set Variable, Queue Task, PIQ and EWT, Routed Notification, Modified Notification, Screen Pop, Close Task, and Resolve Conversation).
...
[block:image]
{
  "images": [
    {
      "image": [
        "https://files.readme.io/31de8c2-Integrations.jpg",
        "Integrations.jpg",
...
[block:image]
{
  "images": [
    {
      "image": [
        "https://files.readme.io/c1a9cfb-Integrations1.jpg",
        "Integrations1.jpg",
        "Screenshot of Manage Prebuilt Integration Page."
      ],
      "align": "center",
      "caption": "Screenshot of Manage Prebuilt Integration Page."
    }
  ]
}
[/block]
